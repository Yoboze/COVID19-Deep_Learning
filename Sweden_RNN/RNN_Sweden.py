...
def run_model(data, name, v, option, cs, epochs, batch_size):
	...
	test_loader = DataLoader(test_data, shuffle=True, batch_size = 5 , drop_last=True)
	model, ep, loss_train = train_model(train_loader,lr,n_hidden, epochs, n_layers, batch_size, out_size, "Train", option = option)
	_, ep, loss_test = train_model(test_loader,lr,n_hidden, epochs, n_layers,5, out_size,"Validation", option = option)
...
def confidence_interval(data, M, option, cs, name, n_steps, DatName):
    ...
    lower =ym -2*(var)
    upper= ym +2*(var)
    dtrange =np.arange(0,122)
    dtrange1 =np.arange(122, 200)
    
    font = 24
    fig, ax = plt.subplots() 
    ax.plot(dtrange, data, 'k--', marker='o', lw=2,label='{}-Data'.format(DatName))
    #ax.plot(dtrange1, test_data,'ob',lw=2,label='Test Data')
    ax.plot(dtrange1, ym,'r-',lw=2,label='{}-Mean-Prediction for Sweden'.format(option))
